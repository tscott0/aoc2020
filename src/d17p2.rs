...
pub fn solve(input: String, cycles: usize) -> usize {
    let mut all_cubes: HashMap<(isize, isize, isize, isize), bool> = HashMap::new();
    ...
    input.lines().enumerate().for_each(|(y, l)| {
        l.trim().chars().enumerate().for_each(|(x, r)| {
            ...
            all_cubes.insert((x as isize, y as isize, 0, 0), r == '#');
        })
    });
    ...
    for c in 0..cycles {
            ...
            for y in min_y..=max_y {
                for z in min_z..=max_z {
                    for w in min_w..=max_w {
                        all_cubes.entry((x, y, z, w)).or_insert_with(|| false);
                    }
                }
            }
        }

        let mut changes: HashMap<(isize, isize, isize, isize), bool> = HashMap::new();

        let cube_copy = all_cubes.clone();
        cube_copy.iter().for_each(|(cube, active)| {
            let (x, y, z, w) = cube.clone();
            min_x = min_x.min(x - 1);
            min_y = min_y.min(y - 1);
            min_z = min_z.min(z - 1);
            min_w = min_w.min(w - 1);
        ...
        changes.iter().for_each(|(k, v)| {
            all_cubes.insert(*k, *v);
        });

        active_count = all_cubes.iter().filter(|(_, &a)| a).count();
...
fn neighbours(origin: (isize, isize, isize, isize)) -> Vec<(isize, isize, isize, isize)> {
    let (ox, oy, oz, ow) = origin;

    let mut neighbours: Vec<(isize, isize, isize, isize)> = vec![];
...
fn example_1() {
    ...
    assert_eq!(solve(input.to_string(), 6), 848);
}
